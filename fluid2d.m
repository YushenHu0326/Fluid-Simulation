...
for i=1:TIME_STEP_TOTAL
    ...
    for j=1:PARTICLE_NUM
        ...
        if is_out_of_bound(particles(j,:))
            particle_v(j,2)=-particle_v(j,2)*0.5;
        else
            % add gravity
    ...
    for j=1:PARTICLE_NUM
        if ~is_out_of_bound(particles(j,:))
            ...
            grid_v(ceil(particles(j,2)),ceil(particles(j,1)),1)=grid_v(ceil(particles(j,2)),ceil(particles(j,1)),1)+particle_v(j,1)/1000;
            grid_v(ceil(particles(j,2)),ceil(particles(j,1)),2)=grid_v(ceil(particles(j,2)),ceil(particles(j,1)),2)+particle_v(j,2)/1000;
        end
    end
